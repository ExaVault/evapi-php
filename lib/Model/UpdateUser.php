...
class UpdateUser implements ArrayAccess
{
    ...
    const PERMISSIONS_UPLOAD = 'upload';
    const PERMISSIONS_DOWNLOAD = 'download';
    const PERMISSIONS_DELETE = 'delete';
    const PERMISSIONS_MODIFY = 'modify';
    const PERMISSIONS__LIST = 'list';
    const PERMISSIONS_CHANGE_PASSWORD = 'changePassword';
    const PERMISSIONS_SHARE = 'share';
    const PERMISSIONS_NOTIFICATION = 'notification';
    ...
    public function getPermissionsAllowableValues()
    {
        return [
            ...
            self::PERMISSIONS_CHANGE_PASSWORD,
            self::PERMISSIONS_SHARE,
            self::PERMISSIONS_NOTIFICATION,
        ];
    }
    ...
    public function __construct(array $data = null)
    {
        ...
        $this->container['destination_folder'] = isset($data['destination_folder']) ? $data['destination_folder'] : null;
    ...
    public function listInvalidProperties()
    {
        ...
        if (!in_array($this->container['permissions'], $allowed_values)) {
            $invalid_properties[] = sprintf(
    ...
    /**
     * Sets user_id
     * @param int $user_id The user's ID. Note that this is our internal ID, and _not the username_. You can obtain it by calling the <a href=\"#operation/getUser\">getUser</a> method.
     * @return $this
     */
    ...
    /**
     * Sets username
     * @param string $username Username of the user. This should follow standard username conventions; e.g. all lowercase, no spaces, etc. We do allow email addresses as usernames.
     * @return $this
     */
    ...
    /**
     * Sets nickname
     * @param string $nickname The user's nickname (e.g. 'David from Sales').
     * @return $this
     */
     ...
     * @param string $destination_folder The path to the user's home folder. For the account root, specify `/`. Otherwise, use standard Unix path format, e.g. `/path/to/some/dir`.
     * @return $this
     */
    ...
    /**
     * Sets role
     * @param string $role The user's role. Note that admin users cannot have a `destinationFolder` other than `/`, and will be setup with full permissions regardless of what you specify in the `permissions` property.
     * @return $this
     */
    ...
    /**
     * Sets permissions
     * @param string $permissions A CSV string of user permissions. For example: `upload,download,list`. Note that users will be unable to see any files in the account unless you include `list` permission.
     * @return $this
     */
    public function setPermissions($permissions)
    {
        $allowed_values = $this->getPermissionsAllowableValues();
        if (!is_null($permissions) && !in_array($permissions, $allowed_values)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value for 'permissions', must be one of '%s'",
                    implode("', '", $allowed_values)
    ...
    /**
     * Sets time_zone
     * @param string $time_zone The user's timezone, used for accurate time display within the application. See <a href='https://php.net/manual/en/timezones.php' target='blank'>this page</a> for allowed values.
     * @return $this
     */
    ...
    /**
     * Sets expiration
     * @param string $expiration Optional date when the user should expire, formatted `YYYY-mm-dd hh:mm:ss`.
     * @return $this
     */
    ...
    /**
     * Sets locked
     * @param bool $locked If true, the user's account is locked.
     * @return $this
     */
